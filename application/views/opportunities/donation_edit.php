    ...
    <div class="row">
        <div class="small-12 medium-9 columns">
            <div class="box">
                <h2>
                    Edit Donation
                     <a href="<?php echo site_url('campaigns/'.$campaign->id_token.'/opportunities/'.$opportunity->id_token.'/donations') ?>" class="right"><i class="fa fa-times-circle action"></i></a>
                </h2>

                <form method="post" action="<?php echo site_url('campaigns/'.$campaign->id_token.'/opportunities/'.$opportunity->id_token.'/donations/'.($donation->id_token ? $donation->id_token.'/edit' : 'new')) ?>">
                    <div class="row">
                        ...
                        <div class="small-12 medium-8 columns">
                            ...
                            <select name="donation_level">
                                ...
                                <?php foreach( $campaign->donation_levels as $level ) : ?>
                                    <option value="<?php echo $level->level_id ?>"<?php if( $donation && $donation->donation_level_id && $donation->donation_level_id == $level->level_id ) : ?> selected<?php endif ?>><?php echo $level->label ?> (<?php echo money_format('%n', $level->amount/100) ?>)</option>
                                <?php endforeach ?>
                            </select>
                            <?php endif ?>
